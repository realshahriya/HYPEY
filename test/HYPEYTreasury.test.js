...
describe("HYPEYTreasury", function () {
  ...
  describe("Token Deposits", function () {
    it("Should allow anyone to deposit tokens", async function () {
      ...
      // Account for 3% burn during transfer (970 tokens received instead of 1000)
      const expectedAmount = amount * 97n / 100n; // 97% after 3% burn
      expect(await mockToken.balanceOf(treasuryAddress)).to.equal(initialBalance + expectedAmount);
    });
  ...
  describe("Token Disbursements", function () {
    it("Should allow owner to disburse tokens", async function () {
      ...
      // Account for 3% burn during transfer (970 tokens received instead of 1000)
      const expectedAmount = amount * 97n / 100n; // 97% after 3% burn
      expect(await mockToken.balanceOf(user1.address)).to.equal(initialBalance + expectedAmount);
    });
